...
export default function CompletedMatchGrid({ players, stats, ratings, ratingWindowEnd }: Props) {
  if (players.length === 0) return null

  const showRating = new Date() > new Date(ratingWindowEnd)
  const statsMap   = new Map(stats.map(s => [s.userId, s]))

  // Pre-compute effective averages for all players
  const avgMap = new Map<string, number | null>(
    players.map(p => [
      p.userId,
      showRating ? effectiveAvg(p.userId, players, ratings) : null,
    ])
  )

  return (
    <div className="rounded-lg border bg-card overflow-x-auto">
      <table className="w-full min-w-[420px]" style={{ fontSize: 12 }}>
        <thead>
